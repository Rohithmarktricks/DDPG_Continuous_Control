...
def get_environment_info(location): 
    ...
    pirnt(f"There are {states.shape[0]} agents. Each observes a state with length; {state_size}")
    print(f"The state for the first agent looks like: {states[0]}")
    return (env, brai_name, brain, action_size, state_size)


def get_agent(state_size, action_size):
    '''Initializes and returns the new agent'''
    agent = Agent(state_size=state_size, action_size=actio_size, random_seed=10)
    return agent


def train_ddpg_agent(env, brain_name, brain, action_size, state_size, agent, num_episodes=1000, max_t=10000):
    '''
    Trains the agent.
    ...
    Repeat steps 1 to 3 untill the episode is done.
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            next_state = env_info.vector_observation[0]             # get the next state
        ...
        if np.mean(scores_deque) >= 31.0:
            ...
            actor_network_path = 'saved_models/checkpoint_actor_'+start_time+'.pth'
            print(f"Saved Actor network weights at {actor_network_path}")
            critic_network_path = 'saved_models/checkpoint_critic_'+start_time+'.pth'
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train DDPG Agent", usage="python train_agent.py <path to Reacher Environment>")
    ...
    # Train the agent.
    train_agent(env, brain_name, brain, action_size, state_size, agent, num_episodes=1000, max_t=10000)
